...
## 1. プロジェクト概要
本プロジェクトは、Raspberry Pi Pico 2（RP2350）を搭載した「Waveshare RP2350-Touch-LCD-2.8」ボード上で動作する、1980年代の Hu-BASIC を土台に、当時の各種 BASIC の機能と本機固有の拡張を加えた BASIC インタプリタ **pico-basic** を開発する。Hu-BASIC の互換実装を目指すものではない。
...
* **音声出力:** I2S DAC（PCM5101A）を PIO + DMA で駆動し、48kHz でサウンドを出力する。PSG 相当の矩形波・3重和音に対応し、発音キューを DMA 割り込みのシーケンサが進める**非同期再生**。起動時には PC-9801 風の 2 音（2000Hz→1000Hz）を鳴らす。
...
* **倍精度実数型 (`#`):** `double` による近似（Hu-BASIC と同じ表記を踏襲）
* **文字列型 (`$`):** 最大長255文字の文字列
* **配列:** 1次元および2次元配列
...
* **グラフィック・テキスト制御（LCD対応）:**
  ...
  * `PSET (x,y), 色`: 点の描画。
  * `LINE (x1,y1)-(x2,y2), 色 [, B|BF]`: 線の描画（`B` で矩形、`BF` で塗りつぶし矩形）。
...
* **サウンド制御（I2S DAC 出力）:**
  ...
  * `SOUND レジスタ, 値`: PSG（AY-3-8910 相当）レジスタへの書き込み。トーン3ch・ノイズ・音量・エンベロープを制御し、明示停止まで鳴り続ける（`PLAY` のキュー再生とは排他）。周波数 = 2MHz / (16 × 周期)。
* **ハードウェア制御（Pico拡張）:**
...
1. **HAL (Hardware Abstraction Layer):** Pico SDKの機能をラップする層。
   ...
   * `hal_sound`: PIO（`src/pio/i2s_out.pio`）+ DMA による I2S 出力。3声分の矩形波をダブルバッファ上でミックスし、DMA 完了割り込みで空いた側を埋める。発音は 128 段のキューに積み、割り込み内のシーケンサがサンプル単位で進めるため、演奏中も BASIC を実行できる。MML の解釈と声の時間軸合成は parser 側（`execute_music`）が担当する。
